  ...
  return this._createGToken(function(err, gToken) {
    if (err) {
      ...
      return gToken.getToken(function (err, token) {
        ...
        var expires_in = Math.round((gToken.expires_at - now)/2); // half the time
        // console.log('token expires_in was ' + expires_in/1000);
        return done(err, {
          access_token: token,
          token_type: 'Bearer',
...
JWT.prototype._createGToken = function(callback) {
  ...
  if (this.gtoken) {
    this.gtoken._configure(options); // resets expires_at (forces refresh)
    return callback(null, this.gtoken);
  } else {
    this.gtoken = this.gToken(options);
    return callback(null, this.gtoken);
  }
};
